...
##
## Windows の操作を emacs のキーバインドで行うための設定（Keyhac版）ver.20160627_03
##
...
#   その他のキーは Windows にそのまま渡されるようになるため IME のショートカットを
#   利用することができる。
...
def configure(keymap):
    ...
    def keyboard_quit():
        ...
        if not keymap.getWindow().getClassName().startswith("EXCEL") and keymap.getWindow().getClassName() != "WebViewHost":
            self_insert_command("Esc")()
        keymap.command_RecordStop()
        if keymap_emacs.is_undo_mode:
    ...
    if use_emacs_ime_mode:
        ...
        def is_emacs_ime_mode(window):
            if keymap_emacs_ime.last_window:
                if keymap_emacs_ime.last_window == window:
                    return True
                else:
        ...
        keymap_emacs_ime = keymap.defineWindowKeymap(check_func=is_emacs_ime_mode)

        # emacs日本語入力モードが開始されたときのウィンドウオブジェクトを格納する変数を初期化する
        keymap_emacs_ime.last_window = None

        # マウスのボタンが離されたときに呼び出されるフック関数を退避する
        keymap.enableHook(True)
        keymap_emacs_ime.hook_mouseup = hook.mouseup
        ...
        def enable_emacs_ime_mode(update=True, toggle=False):
            keymap_emacs_ime.last_window = keymap.getWindow()
            keymap_emacs_ime.last_func = None
        ...
        def disable_emacs_ime_mode(update=True, toggle=False):
            keymap_emacs_ime.last_window = None
            hook.mouseup = keymap_emacs_ime.hook_mouseup
            ei_popBalloon(toggle)
            if update:
                keymap._updateKeymap(keymap.getWindow())
        ...
        def ei_toggle_input_method2(key):
            def _func():
                if keymap_emacs_ime.last_func == delete_backward_char:
        ...
        def ei_record_func(func):
            def _func():
                func()
                keymap_emacs_ime.last_func = func
            return _func
        ...
        def ei_hook_mouseup(x, y, vk):
            disable_emacs_ime_mode()
            keymap_emacs_ime.hook_mouseup(x, y, vk)
            return False
        ...
        for vkey in vkeys():
            ...
            define_key(keymap_emacs_ime, "C-S-" + s_vkey, ei_record_func(self_insert_command("C-S-" + s_vkey)))
            define_key(keymap_emacs_ime, "A-"   + s_vkey, ei_record_func(self_insert_command("A-"   + s_vkey)))
            define_key(keymap_emacs_ime, "A-S-" + s_vkey, ei_record_func(self_insert_command("A-S-" + s_vkey)))

        ## C-S-[a-z] -> C-[a-z]、A-S-[a-z] -> A-[a-z] の置き換え設定（emacsシフトモードの設定）
        if use_emacs_shift_mode:
            for vkey in range(VK_A, VK_Z + 1):
                s_vkey = "(" + str(vkey) + ")"
                define_key(keymap_emacs_ime, "C-S-" + s_vkey, ei_record_func(self_insert_command("C-" + s_vkey)))
                define_key(keymap_emacs_ime, "A-S-" + s_vkey, ei_record_func(self_insert_command("A-" + s_vkey)))

        ## 「IME の切り替え」のキー設定
        define_key(keymap_emacs_ime, "(243)",  ei_toggle_input_method)
        define_key(keymap_emacs_ime, "(244)",  ei_toggle_input_method)
        define_key(keymap_emacs_ime, "A-(25)", ei_toggle_input_method)
        ...
        define_key(keymap_emacs_ime, "C-a", ei_record_func(move_beginning_of_line))
        define_key(keymap_emacs_ime, "C-e", ei_record_func(move_end_of_line))

        define_key(keymap_emacs_ime, "Left",     ei_record_func(backward_char))
        define_key(keymap_emacs_ime, "Right",    ei_record_func(forward_char))
        define_key(keymap_emacs_ime, "Up",       ei_record_func(previous_line))
        define_key(keymap_emacs_ime, "Down",     ei_record_func(next_line))
        define_key(keymap_emacs_ime, "PageUP",   ei_record_func(scroll_up))
        define_key(keymap_emacs_ime, "PageDown", ei_record_func(scroll_down))
        define_key(keymap_emacs_ime, "Home",     ei_record_func(move_beginning_of_line))
        define_key(keymap_emacs_ime, "End",      ei_record_func(move_end_of_line))
        ...
        define_key(keymap_emacs_ime, "Delete", ei_record_func(delete_char))
        define_key(keymap_emacs_ime, "C-d",    ei_record_func(delete_char))
        ...
        if toggle_input_method_key:
            for key in toggle_input_method_key:
                define_key(keymap_emacs_ime, key, ei_toggle_input_method2(key))

        ## 「スクロール」のキー設定（上書きされないように最後に設定する）
        if scroll_key:
            define_key(keymap_emacs_ime, scroll_key[0].replace("M-", "A-"), ei_record_func(scroll_up))
            define_key(keymap_emacs_ime, scroll_key[1].replace("M-", "A-"), ei_record_func(scroll_down))
